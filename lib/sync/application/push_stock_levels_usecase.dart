// Push use case for stock levels using change_log outbox.
import 'package:jaayko/sync/application/_ports.dart';
import 'package:jaayko/sync/application/push_port.dart';
import 'package:jaayko/sync/domain/dtos/stock_level_delta_dto.dart';
...
class PushStockLevelsUseCase implements PushPort {
  ...
  Future<int> execute({int batchSize = 200}) async {
    ...
    Future<Map<String, Object?>?> build(ChangeLogEntry e) async {
      ...
      final dto = StockLevelDeltaDto.fromEntity(
        entity,
        t,
        DateTime.now().toUtc(),
      ).toJson();
    ...
    return outbox.push(
      buildPayload: build,
      postFn: (ds) => api.postStockLevelDeltas(ds),
