// Shared base for Add/Edit customer panels.
// Builds the form, handles focus flow and safe closing, and calls onSubmit
// to actually create/update. Always pops a `Customer` (or null on cancel).
...
typedef CustomerSubmit = Future<Customer> Function(Customer draft);

class CustomerFormBase extends ConsumerStatefulWidget {
  final Customer? initial;
  final String? initialCompanyId;
  final String title;
  final String submitLabel;
  final CustomerSubmit onSubmit;

  /// Optional extras shown on the right side (edit-only actions, etc.)
  final Widget Function(BuildContext context, Customer? initial)? rightExtras;
  ...
  @override
  ConsumerState<CustomerFormBase> createState() => _CustomerFormBaseState();
}

class _CustomerFormBaseState extends ConsumerState<CustomerFormBase> {
  ...
  Future<void> _submit() async {
    ...
    // Build draft entity (Add/Edit share the same builder)
    ...
    try {
      ...
      if (!mounted) return;
      // Close overlays/keyboard then pop with a Customer (right type).
        ...
        child: Scaffold(
          appBar: AppBar(
            ...
            leading: IconButton(
              ...
              // Always return null on cancel (typed)
              onPressed: () => Navigator.of(context).maybePop<Customer?>(null),
            ),
            actions: [
              IconButton(
                ...
                builder: (context, constraints) {
                  ...
                  final right = <Widget>[
                    ...
                    if (isEdit) ...[
                      ...
                      if (widget.rightExtras != null)
                        widget.rightExtras!(context, widget.initial),
                    ],
                  ];
